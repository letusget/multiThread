...
template <typename T> ThreadPool<T>::ThreadPool(int min, int max) {
  ...
  do {
    ...
    //创建线程
    //这里 manager需要是非成员函数，或者作为
    //静态成员，不再属于对象，而是属于类，就有了地址，就可以传递函数地址了
    pthread_create(
        &managerID, NULL, manager,
        this); //管理者线程,
               //使用this传递静态实例对象，manager就可以管理非静态成员了
    for (int i = 0; i < min; i++) {
...
template <typename T> void ThreadPool<T>::threadExit() {
  //获取当前的线程ID
...
//当前线程池中线程个数
template <typename T> int ThreadPool<T>::getBusyNum() {
  //这里有可能会被同时访问，所以需要加锁
...
//获取当前创建的线程个数
template <typename T> int ThreadPool<T>::getAliveNum() {
  pthread_mutex_lock(&mutexPool);
